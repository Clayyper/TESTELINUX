...
const { conferirInformeComCalculo } = require('./comparadorInformeRendimentos');
...
function normalizePayload(payload = {}) {
  ...
  return {
    ...
    ecac: payload.ecac || payload.informe || null,
    ecacConferencia: payload.ecacConferencia || payload.conferenciaEcac || null,
    fgts: payload.fgts || payload.fgtsConferencia || null
  };
}
...
function buildEcacFallbackConferencia(data) {
  if (!data.ecac) return null;
  try { return conferirInformeComCalculo(data.calculo || {}, data.ecac || {}); }
  catch (_) {
    const totais = data.ecac?.totais || {};
    const bruto = safeNum(data.totais?.bruto, 0);
    const rend = safeNum(totais.rendimentoTributavel, 0);
    return { linhas: [
      { item: 'Rendimentos tributáveis declarados no e-CAC', sistema: bruto, ecac: totais.rendimentoTributavel, diferenca: bruto - rend, status: Math.abs(bruto - rend) <= 1 ? 'OK' : 'ALERTA' },
      { item: 'Previdência oficial declarada', sistema: null, ecac: totais.previdenciaOficial, diferenca: null, status: 'INFORMATIVO' },
      { item: 'IRRF sobre rendimentos tributáveis', sistema: null, ecac: totais.impostoRetido, diferenca: null, status: 'INFORMATIVO' },
      { item: '13º / tributação exclusiva', sistema: data.calculo?.rubricas?.decimoTerceiro ?? null, ecac: totais.decimoTerceiroRendimento, diferenca: null, status: 'INFORMATIVO' },
      { item: 'IRRF do 13º', sistema: null, ecac: totais.decimoTerceiroImpostoRetido, diferenca: null, status: 'INFORMATIVO' }
    ], resumo: {}, avisos: ['Conferência gerada diretamente no relatório porque os dados do e-CAC foram enviados sem comparação prévia.'] };
  }
}

function drawEcacComparisonPage(pdf, data) {
  if (!data.ecac) return;
  const ecac = data.ecac || {};
  const conferencia = data.ecacConferencia || buildEcacFallbackConferencia(data) || {};
  const linhas = Array.isArray(conferencia.linhas) && conferencia.linhas.length ? conferencia.linhas : (buildEcacFallbackConferencia(data)?.linhas || []);
  const totais = ecac.totais || {};
  const indicadores = ecac.indicadores || {};
  const fontes = ecac.fontesPagadoras || [];
  const arquivos = ecac.arquivos || [];
  pdf.newPage();
  drawHeader(pdf, '3ª FOLHA - COMPARAÇÃO TRCT (SISTEMA) VS e-CAC / RECEITA FEDERAL');
  let y = 122;
  drawSection(pdf, y, '07 - IDENTIFICAÇÃO DO INFORME e-CAC'); y += 30;
  drawField(pdf, 34, y, 128, 'ANO-CALENDÁRIO', ecac.anoCalendario || '—', 58);
  drawField(pdf, 168, y, 194, 'BENEFICIÁRIO', [ecac.beneficiario?.cpf, ecac.beneficiario?.nome].filter(Boolean).join(' - ') || '—', 78);
  drawField(pdf, 368, y, 193, 'ARQUIVOS LIDOS', String(ecac.quantidadeLidos ?? arquivos.length ?? 1), 65);
  y += 62;
  drawSection(pdf, y, '08 - TOTAIS DECLARADOS NO e-CAC'); y += 30;
  drawField(pdf, 34, y, 166, 'RENDIMENTOS TRIBUTÁVEIS', brl(totais.rendimentoTributavel), 80);
  drawField(pdf, 206, y, 166, 'PREVIDÊNCIA OFICIAL', brl(totais.previdenciaOficial), 76);
  drawField(pdf, 378, y, 183, 'IRRF', brl(totais.impostoRetido), 42);
  y += 54;
  drawField(pdf, 34, y, 166, '13º / EXCLUSIVA', brl(totais.decimoTerceiroRendimento), 68);
  drawField(pdf, 206, y, 166, 'IRRF 13º', brl(totais.decimoTerceiroImpostoRetido), 52);
  drawField(pdf, 378, y, 183, 'TOTAL COM 13º', brl(indicadores.rendimentoTotalCom13), 65);
  y += 70;
  drawSection(pdf, y, '09 - COMPARAÇÃO AUTOMÁTICA: SISTEMA TRCT VS e-CAC'); y += 28;
  const x = 34; const cols = [205, 91, 91, 91, 49];
  pdf.rect(x, y, 527, 24, COLORS.navy, COLORS.navyDark, 0.7);
  ['ITEM', 'SISTEMA/TRCT', 'e-CAC', 'DIFERENÇA', 'STATUS'].forEach((h, i) => pdf.text(h, x + 6 + cols.slice(0, i).reduce((a,b)=>a+b,0), y + 8, { size: 7.4, bold: true, color: COLORS.white }));
  y += 24;
  linhas.slice(0, 9).forEach((r, idx) => {
    const fill = idx % 2 === 0 ? COLORS.white : COLORS.gray2;
    pdf.rect(x, y, 527, 28, fill, COLORS.border, 0.45);
    let c = x;
    pdf.text(String(r.item || r.chave || 'Item').slice(0, 43), c + 6, y + 9, { size: 6.9 }); c += cols[0];
    pdf.textRight(r.sistema == null ? '—' : brl(r.sistema), c + cols[1] - 6, y + 9, { size: 6.9 }); c += cols[1];
    pdf.textRight(r.ecac == null ? '—' : brl(r.ecac), c + cols[2] - 6, y + 9, { size: 6.9 }); c += cols[2];
    pdf.textRight(r.diferenca == null ? '—' : brl(r.diferenca), c + cols[3] - 6, y + 9, { size: 6.9 }); c += cols[3];
    pdf.text(statusLabel(r.status), c + 5, y + 9, { size: 6.8, color: statusColor(r.status === 'ALERTA' ? 'DIVERGENTE' : r.status), bold: true });
    y += 28;
  });
  y += 14;
  drawSection(pdf, y, '10 - FONTES PAGADORAS / OBSERVAÇÕES'); y += 28;
  const fonteLines = fontes.length ? fontes.slice(0, 4).map((f) => `${f.cnpj || 'CNPJ não lido'} - ${f.nome || 'Nome não lido'}`) : ['Fonte pagadora não localizada automaticamente.'];
  fonteLines.forEach((line) => { pdf.text(`• ${line}`.slice(0,112), 44, y, { size: 7.6 }); y += 16; });
  (conferencia.avisos || []).slice(0,4).forEach((aviso) => { pdf.text(`• ${aviso}`.slice(0,112), 44, y, { size: 7.4, color: COLORS.warn }); y += 16; });
  y = Math.max(y + 10, 735);
  pdf.rect(34, y, 527, 48, COLORS.blueLight, COLORS.border, 0.7);
  const temAlerta = linhas.some((r) => ['ALERTA','DIVERGENTE'].includes(String(r.status).toUpperCase()));
  pdf.text('CONCLUSÃO AUTOMÁTICA', 44, y + 8, { size: 8.5, bold: true, color: COLORS.navyDark });
  pdf.text(temAlerta ? 'Foram identificadas diferenças entre o cálculo TRCT do sistema e os valores declarados no e-CAC. Revisar período, fontes pagadoras e rubricas tributáveis.' : 'Não foram identificadas divergências automáticas relevantes nos itens comparáveis enviados ao relatório.', 44, y + 26, { size: 7.4, color: temAlerta ? COLORS.bad : COLORS.ok, bold: true });
}


function drawFgtsComparisonPage(pdf, data) {
  if (!data.fgts) return;
  const fgts = data.fgts || {};
  const linhas = Array.isArray(fgts.linhas) && fgts.linhas.length ? fgts.linhas : [
    { item: 'Saldo FGTS informado/importado', sistema: null, fgts: fgts.saldo, diferenca: null, status: fgts.saldo == null ? 'NAO_LIDO' : 'INFORMATIVO' },
    { item: 'Multa 40% esperada sobre saldo FGTS', sistema: fgts.multaSistema, fgts: fgts.multaEsperada, diferenca: fgts.diferenca, status: fgts.status || 'INFORMATIVO' },
    { item: 'Multa FGTS do TRCT/sistema', sistema: fgts.multaSistema, fgts: null, diferenca: null, status: fgts.multaSistema ? 'INFORMATIVO' : 'NAO_LIDO' }
  ];

  pdf.newPage();
  drawHeader(pdf, '4ª FOLHA - CONFERÊNCIA FGTS / CAIXA');

  let y = 122;
  drawSection(pdf, y, '11 - SALDO FGTS E MULTA RESCISÓRIA'); y += 30;
  drawField(pdf, 34, y, 166, 'SALDO FGTS', brl(fgts.saldo), 72);
  drawField(pdf, 206, y, 166, 'MULTA 40% ESPERADA', brl(fgts.multaEsperada), 80);
  drawField(pdf, 378, y, 183, 'MULTA NO TRCT/SISTEMA', brl(fgts.multaSistema), 80);
  y += 68;
  drawField(pdf, 34, y, 166, 'DIFERENÇA', brl(fgts.diferenca), 62);
  drawField(pdf, 206, y, 166, 'STATUS', statusLabel(fgts.status), 58);
  drawField(pdf, 378, y, 183, 'ORIGEM', fgts.origem || 'manual/importado', 64);

  y += 72;
  drawSection(pdf, y, '12 - COMPARAÇÃO AUTOMÁTICA: SISTEMA TRCT VS FGTS'); y += 28;
  const x = 34; const cols = [215, 94, 94, 78, 46];
  pdf.rect(x, y, 527, 24, COLORS.navy, COLORS.navyDark, 0.7);
  ['ITEM', 'SISTEMA/TRCT', 'FGTS/CAIXA', 'DIFERENÇA', 'STATUS'].forEach((h, i) => pdf.text(h, x + 6 + cols.slice(0, i).reduce((a,b)=>a+b,0), y + 8, { size: 7.2, bold: true, color: COLORS.white }));
  y += 24;

  linhas.slice(0, 8).forEach((r, idx) => {
    const fill = idx % 2 === 0 ? COLORS.white : COLORS.gray2;
    pdf.rect(x, y, 527, 30, fill, COLORS.border, 0.45);
    let c = x;
    pdf.text(String(r.item || 'Item').slice(0, 46), c + 6, y + 10, { size: 6.9 }); c += cols[0];
    pdf.textRight(r.sistema == null ? '—' : brl(r.sistema), c + cols[1] - 6, y + 10, { size: 6.9 }); c += cols[1];
    pdf.textRight(r.fgts == null ? '—' : brl(r.fgts), c + cols[2] - 6, y + 10, { size: 6.9 }); c += cols[2];
    pdf.textRight(r.diferenca == null ? '—' : brl(r.diferenca), c + cols[3] - 6, y + 10, { size: 6.9 }); c += cols[3];
    pdf.text(statusLabel(r.status), c + 5, y + 10, { size: 6.5, color: statusColor(r.status === 'ALERTA' ? 'DIVERGENTE' : r.status), bold: true });
    y += 30;
  });

  y += 18;
  drawSection(pdf, y, '13 - ORIGEM DO SALDO / OBSERVAÇÕES'); y += 28;
  const obs = [
    fgts.arquivo?.nome ? `Arquivo importado: ${fgts.arquivo.nome}` : 'Saldo informado manualmente.',
    fgts.encontradoPor ? `Saldo localizado por: ${fgts.encontradoPor}` : null,
    ...(fgts.avisos || []),
    ...(fgts.observacoes || [])
  ].filter(Boolean);

  obs.slice(0, 8).forEach((item) => { pdf.text(`• ${String(item).slice(0, 112)}`, 44, y, { size: 7.5, color: COLORS.ink }); y += 16; });

  y = Math.max(y + 10, 735);
  pdf.rect(34, y, 527, 48, COLORS.blueLight, COLORS.border, 0.7);
  const divergente = String(fgts.status || '').toUpperCase() === 'DIVERGENTE';
  pdf.text('CONCLUSÃO AUTOMÁTICA FGTS', 44, y + 8, { size: 8.5, bold: true, color: COLORS.navyDark });
  pdf.text(divergente ? 'Foi identificada diferença entre a multa FGTS do sistema/TRCT e a multa de 40% calculada sobre o saldo FGTS informado/importado.' : 'Não foi identificada divergência relevante entre a multa FGTS esperada e a multa FGTS do sistema/TRCT.', 44, y + 26, { size: 7.4, color: divergente ? COLORS.bad : COLORS.ok, bold: true });
}
...
async function createRelatorioBuffer(payload = {}) {
  ...
  drawEcacComparisonPage(pdf, data);
  drawFgtsComparisonPage(pdf, data);
  return pdf.build();
}
